...
//    --------------------*/
    ...
    $(".movie__item").hover(function(){
      $(this).toggleClass("border-success border-dark");
  });

  var rating_input = document.getElementById('rating');
  if (rating_input)
  {
    const max = +rating_input.getAttribute("max");
    rating_input.addEventListener("keydown", function(e) {
        const typed = +e.key;
    if(!isNaN(typed)) e.preventDefault(); // Allow any non-number keys (backspace etc.)
    if ( +(e.target.value + typed) <= max) {
        rating_input.value += typed
    }});
  }
